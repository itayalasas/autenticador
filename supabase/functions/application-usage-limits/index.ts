import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { createClient } from 'npm:@supabase/supabase-js@2.43.2';
import { resolveScopedPlanEntitlements } from '../_shared/application-billing.ts';
import { normalizeBillingEnvironmentName } from '../_shared/mercadopago.ts';

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
  'Access-Control-Allow-Methods': 'POST, OPTIONS',
  'Access-Control-Max-Age': '86400',
};

interface UsageLimitsRequest {
  application_id: string;
  api_key: string;
  tenant_id?: string;
  app_user_id?: string;
}

function jsonResponse(body: unknown, status = 200) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}

function findFeatureValue(features: any[], code: string): string | null {
  const feature = features.find((item) => item?.code === code);
  return feature ? String(feature.value) : null;
}

Deno.serve(async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response(null, { status: 200, headers: corsHeaders });
  }

  try {
    if (req.method !== 'POST') {
      return jsonResponse({
        success: false,
        error: { code: 'METHOD_NOT_ALLOWED', message: 'Only POST method is allowed' },
      }, 405);
    }

    const supabase = createClient(
      Deno.env.get('SUPABASE_URL') ?? '',
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '',
    );

    const body = await req.json().catch(() => ({})) as UsageLimitsRequest;
    const applicationId = String(body.application_id || '').trim();
    const apiKey = String(body.api_key || '').trim();
    const tenantId = String(body.tenant_id || '').trim() || null;
    const appUserId = String(body.app_user_id || '').trim() || null;

    if (!applicationId || !apiKey) {
      return jsonResponse({
        success: false,
        error: { code: 'MISSING_FIELDS', message: 'application_id y api_key son requeridos' },
      }, 400);
    }

    if (!tenantId && !appUserId) {
      return jsonResponse({
        success: false,
        error: { code: 'MISSING_SCOPE', message: 'Debes enviar tenant_id o app_user_id' },
      }, 400);
    }

    const { data: application, error: applicationError } = await supabase
      .from('applications')
      .select('id, application_id, name')
      .eq('application_id', applicationId)
      .maybeSingle();

    if (applicationError || !application) {
      return jsonResponse({
        success: false,
        error: { code: 'APPLICATION_NOT_FOUND', message: 'Aplicacion no encontrada' },
      }, 404);
    }

    const { data: apiKeyData } = await supabase
      .from('api_keys')
      .select('application_id, is_active, environment')
      .eq('key_hash', apiKey)
      .eq('is_active', true)
      .maybeSingle();

    if (!apiKeyData) {
      return jsonResponse({
        success: false,
        error: { code: 'INVALID_API_KEY', message: 'API Key invalida o inactiva' },
      }, 401);
    }

    if (apiKeyData.application_id !== application.id) {
      return jsonResponse({
        success: false,
        error: { code: 'API_KEY_MISMATCH', message: 'API Key no pertenece a esta aplicacion' },
      }, 403);
    }

    const billingEnvironment = normalizeBillingEnvironmentName(apiKeyData.environment || null);

    const { subscription, plan, entitlements } = await resolveScopedPlanEntitlements({
      supabase,
      application,
      tenantId,
      appUserId,
      environmentName: billingEnvironment,
    });

    const features = entitlements.features || [];

    const totalEmailsValue = findFeatureValue(features, 'total_de_correos_mensuales');
    const pdfGenerationsValue = findFeatureValue(features, 'pdf_generations_monthly');
    const emailOveragePriceValue = findFeatureValue(features, 'email_overage_price');
    const pdfOveragePriceValue = findFeatureValue(features, 'pdf_overage_price');

    return jsonResponse({
      success: true,
      data: {
        plan_id: plan?.id || null,
        plan_name: plan?.name || null,
        currency: plan?.currency || 'UYU',
        limits: {
          total_de_correos_mensuales: totalEmailsValue !== null ? Number(totalEmailsValue) : null,
          pdf_generations_monthly: pdfGenerationsValue !== null ? Number(pdfGenerationsValue) : null,
        },
        overage_prices: {
          email_overage_price: emailOveragePriceValue !== null ? Number(emailOveragePriceValue) : 0,
          pdf_overage_price: pdfOveragePriceValue !== null ? Number(pdfOveragePriceValue) : 0,
        },
        period: {
          start: subscription?.current_period_start || null,
          end: subscription?.current_period_end || null,
        },
        subscription_status: subscription?.status || null,
        payer_email: subscription?.payer_email || null,
      },
    });
  } catch (error: any) {
    console.error('application-usage-limits error:', error);
    return jsonResponse({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: error?.message || 'Error interno del servidor' },
    }, 500);
  }
});
